...
\chapter{Introdução}
    ...
    Perante esta realidade, o presente trabalho apresenta um caso de estudo focado no desenvolvimento de um assistente virtual baseado em Modelos de Linguagem de Grande Escala (LLM - Large Language Models). O objetivo principal é capacitar utilizadores comuns na otimização dos seus espaços domésticos através de capacidades de visão computacional: o sistema analisa imagens reais fornecidas pelo utilizador e gera recomendações personalizadas para o melhor aproveitamento de cada divisão.
    
    A motivação técnica para este projeto reside na exploração das potencialidades do Ollama e na implementação de ferramentas de pipeline que garantem a interoperabilidade do sistema. Esta arquitetura permite a alternância entre diferentes modelos de inteligência artificial de forma dinâmica, garantindo que o assistente possa evoluir e adaptar-se a diferentes requisitos de processamento e análise contextual.
    ...
    \section{Apresentação do Caso de Estudo}
        O presente caso de estudo foca-se no desenvolvimento de um assistente virtual baseado em Modelos de Linguagem de Grande Escala (LLM - Large Language Models). O objetivo principal é capacitar utilizadores comuns na otimização de ambientes domésticos. Através de capacidades de visão computacional, o sistema analisa imagens reais fornecidas pelo utilizador, gerando recomendações personalizadas para o melhor aproveitamento e funcionalidade do espaço.
    \section{Motivação e Objectivos}
        A motivação deste trabalho reside na exploração das potencialidades dos modelos de linguagem de grande escala (LLM), recorrendo à plataforma Ollama. O projeto foi impulsionado pela oportunidade de implementar ferramentas de pipeline que garantem a interoperabilidade do sistema, permitindo a alternância e o teste de diferentes modelos de IA de forma dinâmica e eficiente durante a execução.
...
\chapter{Desenvolvimento}
    ...
    \begin{enumerate}
        \item Arquitetura da Interface (Front-end)
        Foi implementada uma interface web utilizando a biblioteca Hyperdiv, selecionada pela sua capacidade de gerir estados de forma reativa e minimalista. Esta camada permite ao utilizador uma interação intuitiva, suportando o envio de mensagens de texto, a submissão de ficheiros de imagem e a gestão dinâmica do histórico da sessão.
        \item Orquestração do Agente (Back-end)
        O núcleo do sistema baseia-se numa arquitetura de grafos de estado, utilizando a biblioteca LangGraph. Esta abordagem permitiu definir o comportamento do assistente através de uma pipeline inteligente composta por quatro nós especializados:
        Decisão (Router): Triagem automática entre fluxos puramente textuais ou multimodais.
        Processamento Visual: Extração de características e descrição técnica de ambientes a partir de imagens.
        Processamento Textual: Gestão de consultas diretas e manutenção da continuidade dialógica.
        Refinamento: Síntese final que integra o contexto visual com as diretrizes de design de interiores.
        \item Integração de Modelos e Performance
        A execução dos modelos é suportada pela plataforma Ollama, garantindo flexibilidade na escolha e alternância de modelos (como o LLaMA). A estratégia de desenvolvimento privilegiou a eficiência, isolando o processamento visual para garantir respostas rápidas, e utilizando a técnica de System Prompting para assegurar que todos os conselhos técnicos são entregues de forma profissional e localizados para o português de Portugal.
    ...
    \end{enumerate}
    \section{Dataset, Documentos e Preparação}
        O sistema opera sobre dados não estruturados, especificamente texto (perguntas do utilizador) e imagens (fotos de divisões).
        \begin{itemize}
            \item Fontes de Conhecimento: O conhecimento não advém de um dataset estático, mas sim do vasto corpus de treino dos modelos LLaMA 3 (conhecimento linguístico e de design) e LLaVA (capacidade de visão computacional).
        ...
        \begin{table}[!h]
            ...
            \begin{tabular}{|p{3.4cm}|p{4cm}|p{5cm}|}
               ...
               \rowcolor{gray!20!white}
                ...
                \hline
                langchain-ollama & >=1.0.1,<2.0.0 & Integração para iteração com Ollama \\
                \hline
            \end{tabular}
            \caption{Dependências Utilizadas Pelo Projecto.}
        \end{table}

        Além do python e das bibliotecas, fez se o uso dos seguintes componentes:
        \begin{itemize}
            \item Ollama (LLaMA 3 e LLaVA): Permite a execução de LLMs locais, garantindo a privacidade e segurança dos dados sensíveis do utilizador (fotos da casa), em conformidade com o GDPR.
            \item LangGraph: Utilizado para orquestrar o fluxo de conversação através de um Grafo de Estados. Esta abordagem permite gerir o histórico (memória interna) e implementar um roteamento condicional eficaz.
            \item Hyperdiv: Escolhido pela reatividade da interface e facilidade de construção de interfaces web.
            \item Docker: Escolhido pela portabilidade do sistema em qualquer ambiente.
        \end{itemize}

    \subsection{Por que razão esta abordagem?}
        Esta abordagem de IA Generativa permite a criação de conteúdo original e contextualmente relevante. Ao contrário de sistemas fixos, o uso de modelos baseados em Transformers permite capturar dependências de longo alcance e fornecer respostas semânticas profundas.
    ...
    \section{Arquitetura e Desenvolvimento}
        ...
        \begin{enumerate}
            \item Frontend (HTML5/Hyperdiv): Gere o estado reativo e o carregamento de imagens.
            \item Backend (Python/Hyperdiv): Receciona as mensagens e aciona a pipeline do chatbot.
            \item Serviço de Chatbot (LangGraph): Pipeline que efetua a orquestração do chat.
            \item Modelos de IA (Ollama): Atuam como o motor de inferência profunda.
        \end{enumerate}
        ...
        \subsection{Interface de Utilizador (Front-end)}
        Foi desenvolvida uma interface web minimalista utilizando a biblioteca Hyperdiv. Esta interface foi desenhada para proporcionar uma interação fluida, oferecendo as seguintes funcionalidades:
        ...
        \subsection{Arquitetura do Back-end e Fluxo de Execução}
        O núcleo da aplicação reside no back-end, onde cada interação do utilizador é processada por um agente inteligente. A lógica de execução é orquestrada através de uma pipeline construída com a biblioteca LangGraph, que permite definir o comportamento do agente como um grafo cíclico de estados.
        ...
        \begin{enumerate}
            ...
            \item \textbf{Textual:} Este nó é ativado sempre que a entrada do utilizador consiste exclusivamente em texto. O processamento é delegado ao modelo LLaMA, sendo a interação estruturada através de uma instrução de sistema (System Prompt) rigorosa que define o comportamento e o domínio de atuação do agente.
            \begin{enumerate}
        ...
        Para uma melhor compreensão da dinâmica de execução e das ramificações de decisão do sistema, apresenta-se a figura \ref{fig:diagrama_fluxo} com o fluxograma do agente de conversação. Este diagrama ilustra a orquestração entre os nós de decisão (routing), processamento de visão e refinamento textual, evidenciando como o estado é gerido ao longo da pipeline do \textit{LangGraph}.
...
\chapter{Resultados e Avaliação}
    \section{Resultados}
        ...
        Embora o sistema se tenha revelado funcional, o desempenho foi condicionado pela infraestrutura de execução. Devido à utilização do Ollama em ambiente Docker sem aceleração por GPU, registou-se uma latência considerável na geração de respostas. No entanto, este atraso não comprometeu a precisão do modelo, servindo como uma Prova de Conceito para fins acadêmicos bem-sucedido.
    ...
    \section{Discussão Crítica}
        ...
        Latência de Processamento: Observou-se um tempo de resposta significativo (entre 10 a 30 segundos) no processamento de imagens. Este atraso deve-se à elevada carga computacional exigida pelos modelos de visão em hardware local.
        Persistência de Dados: Atualmente, a memória de sessão é volátil. A ausência de uma base de dados dedicada para o histórico de longo prazo implica que o estado da conversação se perca caso a página seja recarregada.
        Dependência de Hardware: A execução fluida do sistema impõe requisitos técnicos elevados (mínimo de 8GB a 16GB de RAM), o que pode limitar a acessibilidade da ferramenta em dispositivos menos potentes.

        \subsection{Análise Comparativa: Local vs. Cloud}
...
\chapter{Conclusões e Trabalho Futuro}
    ...
    \begin{itemize}
        \item Implementação de RAG (Retrieval-Augmented Generation): Integrar bases de dados externas para garantir que o assistente utiliza conhecimentos técnicos e normativas arquitetónicas atualizadas.
        \item Otimização de Hardware: Migrar a execução para ambientes com suporte CUDA para reduzir drasticamente os tempos de resposta.
        \item Persistência de Longo Prazo: Desenvolver uma camada de base de dados para manter o histórico entre sessões, permitindo um acompanhamento contínuo de projetos de renovação.
    \end{itemize}
...
\nomenclature[01]{\textbf{LLM}}{Large Language Model - Modelo de Linguagem Grande}
\nomenclature[02]{\textbf{LLaMA}}{Modelo de linguagem de texto da Meta}
\nomenclature[03]{\textbf{LLaVA}}{Large Language and Vision Assistant - Modelo multimodal}
\nomenclature[03]{\textbf{GPU}}{Graphic Process Unit - Unidade de Processamento Gráfico}
\nomenclature[03]{\textbf{PoC}}{Proof of Concept - Prova de Conceito}
\nomenclature[03]{\textbf{RAG}}{Retrieval-Augmented Generation - Geração Aumentada via Recuperação}
\nomenclature[03]{\textbf{CUDA}}{Compute Unified Device Architecture - Arquitetura de Dispositivo de Computação Unificada)}
